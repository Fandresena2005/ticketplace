...
CONFIG = {
    ...
    "interval_seconds": 30,  # vérifie toutes les 30 secondes
    "important_keywords": [
...
def load_seen():
    if os.path.exists(CONFIG["seen_events_file"]):
        with open(CONFIG["seen_events_file"], "r") as f:
...
def termux_notify(title, message):
    """Notification Android via Termux"""
    try:
        os.system(f'termux-notification --title "{title}" --content "{message}" --sound')
    except Exception:
        pass
...
def main():
    ...
    while True:
        ...
        try:
            ...
            if important_events or new_events:
                print(f"   🔥 {len(important_events)} importants | 🆕 {len(new_events)} nouveaux")
